...
import {
  ...
  resetSelectedNotificationsEvent,
...
const AdminPanelNotificationsPage = () => {
  ...
  const [selectedNotifications, deleteAllNotifications] = useUnit([
    $selectedNotifications,
    deleteAllNotificationsEvent,
  ]);
  const [selectOneNotification, selectAllNotifications] = useUnit([
    selectOneNotificationEvent,
    selectAllNotificationsEvent,
  ]);
          ...
          <div className={"w-fit flex flex-row items-baseline gap-4"}>
            {selectedNotifications.length > 0 && (
              <div className={"flex flex-row items-baseline gap-4"}>
                <Text text={`Выбрано ${selectedNotifications.length}`} className={"text-text-gray"} />
                <TextButton
            ...
            <TextButton onClick={selectAllNotifications} text={"Выбрать всё"} />
          </div>
        }
        rightContent={<Button onClick={deleteAllNotifications} text={"Удалить"} buttonType={"SECONDARY"} size={"sm"} />}
      />
