...
import AdminPanelHeaderButtonRow from "@/components/organisms/rows/admin-panel-header-button-row/AdminPanelHeaderButtonRow";
import { useAdminPanelHeaderButtonRow } from "@/components/organisms/rows/admin-panel-header-button-row/AdminPanelHeaderButtonRow.hooks";
...
const AdminPanelSalesPage = () => {
  ...
  const handleEditSale = (tableRow: ProductTableRow<ResponseShortSale>) =>
    router.push(pathname.concat(`/${tableRow.id}/edit`));
  ...
  return (
    <React.Fragment>
      {saleIdToDelete && <DeleteAminSalePopup onClose={() => setSaleIdToDelete(null)} tableRow={saleIdToDelete} />}
      ...
      />
      <HeaderRow className={"w-full"} theme={"bordered"} header={"Акции"} />
      <ProductContentTable
