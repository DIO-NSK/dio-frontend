...
const rowCV = ["w-full flex flex-row gap-5 pb-7", "border-b-2 border-light-gray"];
  ...
  <React.Fragment>
    {Array.from({ length: Math.round(dataGrid.length / (blockSize ?? BLOCK_SIZE)) }, (_, blockIndex) => (
      <div key={blockIndex} className={cn(rowCV)}>
        {dataGrid
          .slice(blockIndex * (blockSize ?? BLOCK_SIZE), (blockIndex + 1) * (blockSize ?? BLOCK_SIZE))
...
const OrderPageCRMLogsBlock = ({ orderId }: { orderId: number }) => {
  ...
  useEffect(() => {
    api
      .get("/admin/bucket/order-logs", { params: { orderId: orderId } })
  ...
  return (
    <OrderPageHeaderBlock header={"Состав заказа"}>
      ...
      <div className={"w-full flex flex-col py-5 gap-10"}>
        {isOpen &&
          order.productItemDtoList.map((group) => (
            <ShoppingCartProductCard hasLink={false} card={group as unknown as ResponseCartItem} canInteract={false} />
          ))}
        <TextButton text={isOpen ? "Свернуть" : "Смотреть все"} onClick={handleSwitchOpenState} />
      </div>
    </OrderPageHeaderBlock>
  );
};
...
const OrderCommentBlock = () => {
  ...
  const infoData: HeaderDescription[] = [{ header: "Комментарий", description: comment }];
