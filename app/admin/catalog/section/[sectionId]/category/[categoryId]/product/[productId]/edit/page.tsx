...
import AdminPanelExternalPropertiesBlock from "@/components/organisms/blocks/admin-panel-filled-properties-block/AdminPanelExternalPropertiesBlock";
import AdminPanelFilledPropertiesBlock from "@/components/organisms/blocks/admin-panel-filled-properties-block/AdminPanelFilledPropertiesBlock";
import AdminPanelPhotoBlock from "@/components/organisms/blocks/admin-panel-photo-block/AdminPanelPhotoBlock";
import AdminPanelProductInputGrid from "@/components/organisms/blocks/admin-panel-product-input-grid/AdminPanelProductInputGrid";
...
const editMessage: string = `На данный момент нельзя отредактировать группу и код товара.
     У каждого товара уникальный код и группа.`;

const productOfTheDayDescription: string = `Если чекбокс включён, то данный товар будет отображаться
     в каталоге с уникальным тегом «Товар дня»`;

const textAreaDescription: string =
  "Придумайте привлекающее описание товара. Идеальная длина описания — 3 предложения.";
...
const AdminPanelEditProductPage = ({
  params,
}: {
  params: {
    categoryId: number;
  ...
  const [categoryData, productDetails, getProductDetails] = useUnit([
    $propertiesToEdit,
    $productDetails,
    getProductDetailsEvent,
  ]);
  ...
  const rowCV = ["items-end px-7 w-full grid grid-cols-3", "gap-7 pb-7 border-b-2 border-light-gray"];
  ...
  return (
    <React.Fragment>
      ...
      <FormProvider {...editProductMethods}>
        <Form>
          <HeaderRow className={"w-full"} theme={"bordered"} header={"Редактирование товара"} hasBackIcon />
          <div className={cn(rowCV)}>
            ...
            <ControlledTextInput
              ...
              name={"crmGroup"}
            />
            <Text className={"text-text-gray"} text={editMessage} />
          </div>
