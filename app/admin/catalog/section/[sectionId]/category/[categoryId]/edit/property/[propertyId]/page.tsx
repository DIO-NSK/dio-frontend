...
const ControlledProductList = () => {
  ...
  return (
    <FormBlock
      header={"Товары"}
      leftContent={<Text className={"text-text-gray"} text={`${property?.productsProperties.length} шт.`} />}
      rightContent={<ChevronButton isExpanded={expandedToggle.state} setExpanded={expandedToggle.toggleState} />}
    >
      <section className={"col-span-full flex flex-col gap-7"}>
        {expandedToggle.state &&
          property?.productsProperties.map((product, key, array) => (
...
const EditCategoryPropertyForm = () => (
  <React.Fragment>
    <FormBlock header={"Характеристики свойства"}>
      <ControlledTextInput name={"propertyName"} placeholder={"Введите название характеристики"} />
      <ControlledTextInput name={"propertyValueName"} placeholder={"Введите единицу характеристики"} />
      <ControlledSelectInput name={"propertyType"} placeholder={"Выберите тип характеристики"} items={selectItems} />
    </FormBlock>
    <ControlledProductList />
  </React.Fragment>
);

const AdminEditCategoryPropertyPage = ({
  params,
}: {
  params: {
    propertyId: string;
      ...
      <FormProvider {...methods}>
        ...
        <Form>
          <HeaderRow
            ...
            hasBackIcon
            leftContent={
              <HeaderPropertyRow categoryName={property.categoryName} propertyName={property.propertyName} />
            }
          />
