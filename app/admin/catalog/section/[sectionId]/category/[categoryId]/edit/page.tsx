...
const AdminEditCategoryPage = ({
  ...
  const [pageDidMount, changeCategory, formData] = useUnit([
    editCategoryPageDidMountEvent,
    changeCategoryFx,
    $formData,
  ]);
  ...
  return (
    <React.Fragment>
      ...
      />
      <HeaderRow className={"w-full"} theme={"bordered"} header={"Редактирование категории"} hasBackIcon />
      {formData && (
        <FormProvider {...methods}>
          <Form className={"flex flex-col sm:gap-7"}>
            ...
            <AdminPanelCharBlock blockName={"properties"} disabled />
