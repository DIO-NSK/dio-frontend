...
const ControlledProductList = () => {
  ...
  return (
    <FormBlock
      header={"Товары"}
      leftContent={<Text className={"text-text-gray"} text={`${propertyProducts?.length} шт.`} />}
    ...
    >
      <section className={"col-span-full flex flex-col gap-7"}>
        {expandedToggle.state &&
          propertyProducts?.map((product, key, array) => (
...
const AdminNewCategoryPropertyPage = ({
  params,
}: {
  params: {
    categoryId: number;
  ...
  const propertyProducts = useUnit($categoryPropertyProducts);
  const [createProperty, getPropertyProducts] = useUnit([
    createCategoryPropertyStateFx,
    getCategoryPropertyProductsEvent,
  ]);
