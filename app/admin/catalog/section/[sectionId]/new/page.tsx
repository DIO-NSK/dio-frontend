...
const AdminPanelNewCategoryPage = ({ params }: { params: { sectionId: number } }) => {
  const router = useRouter();

  const [categories, createCategory, creationStatus] = useUnit([$categories, createCategoryFx, $creationStatus]);
  ...
  const methods = useForm<CreateCategoryData>({
    ...
    defaultValues: {
      name: "",
      isNeedParsing: false,
      properties: [defaultCharacteristicData, { ...defaultCharacteristicData, sequenceNumber: 2 }],
    },
    mode: "onSubmit",
  });
  ...
  return (
    <React.Fragment>
      <HeaderRow className={"w-full"} theme={"bordered"} header={"Новая категория"} hasBackIcon />
        ...
        <Form>
          ...
          <div className={"px-7 flex flex-row items-center gap-5"}>
            ...
            {creationStatus === "failure" && (
              <Text text={"Возникли ошибки при создании категории"} className={"text-sm text-red-500"} />
            )}
          </div>
        </Form>
      </FormProvider>
