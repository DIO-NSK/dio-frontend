...
const MobileOrderServicePage = () => {
  ...
  const messageSnackbar = requestSuccess
    ? "В скором времени с вами свяжется специалист"
    : "Заполните данные заново и попробуйте снова";
  ...
  return (
    <InnerPageWrapper>
      ...
      />
      <HeaderRow theme={"bordered"} rightContent={<FiX onClick={router.back} />} header={"Заказать услугу"} />
      <FormProvider {...methods}>
        <Form className={"gap-4"}>
          {inputData.map((input, key) => (
            <ControlledTextInput disabled={isSubmitting} {...input} key={key} />
          ))}
          <ControlledTextArea labelText={"Комментарий"} placeholder={"Введите здесь"} name={"message"} />
