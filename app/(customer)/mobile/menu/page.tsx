...
const MobileMenuPage = () => {
  ...
  const menuTabData: IconTextAction[] = [
    ...
    },
    {
      ...
      action: () => (userCredentials ? handleTabClick("/favorites") : navigation.push("/mobile/authorization")),
      text: "Избранное",
    },
    {
  ...
  return (
    <MobileNavbarWrapper sticky={false}>
      ...
      {userCredentials && (
        <section className={"w-full flex flex-col gap-7 pb-7 border-b-2 border-light-gray"}>
      ...
      <Button text={"Заказать звонок"} onClick={handleOrderCallRequest} buttonType={"SECONDARY"} />
      ...
      <div className={"w-full flex flex-col gap-3"}>
        ...
        <div className={"w-full flex flex-col gap-5"}>
          {infoData.map((row, rowKey) => (
            <Link className={"w-full flex flex-row items-center gap-4"} href={row.href} key={rowKey}>
