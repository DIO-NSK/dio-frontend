...
const MobileLoginByNumberPage = () => {
  ...
  const [loginByPhone, popupDidMount, loginByPhoneError] = useUnit([
    loginByPhoneFx,
    loginByPhonePopupDidMountEvent,
    $loginByPhoneError,
  ]);
    ...
    <InnerPageWrapper classNames={{ mobileWrapper: "gap-5" }}>
      <FormProvider {...methods}>
        ...
        <div className={"w-full flex flex-col gap-3"}>
          {loginMessage && <Text className={"text-sm text-red-500"} text={loginByPhoneError} />}
          <Button
          ...
          />
          <Button onClick={handleLoginByPassword} text={"Войти с помощью пароля"} buttonType={"SECONDARY"} />
        </div>
        <ControlledCaptcha
          key={key}
