...
const noOrdersMessage =
  "Похоже, что Вы еще не сделали ни одного заказа.\n" +
...
const UserProfileOrdersPage = () => {
  ...
  const [activeItem, setActiveItem] = useState<SelectItem<OrderFilters>>(dropdownItems[0]);
  ...
  const sortedOrders = useMemo(() => {
    if (orders.length)
      switch (activeItem.value) {
        ...
        case "price":
          const getTotalPrice = (products: ProfileOrderItem[]) =>
            products.reduce(
              (acc, product) => acc + product.price * product.quantity * (1 - 0.01 * product.discountPercent),
              0,
            );
        ...
        case "amount":
          const getTotalAmount = (products: ProfileOrderItem[]) =>
  ...
  return (
    <UserProfileWrapper>
        ...
        rightContent={
          <div className="flex flex-row items-center gap-5">
            <SelectInput
              ...
              size={"sm"}
            />
            <FiX size={"20px"} className={"w-fit lg:hidden"} onClick={() => navigation.push("/")} />
          </div>
        }
      />
      <SelectInput items={dropdownItems} className={"md:hidden"} selectedItem={activeItem} onSelect={setActiveItem} />
      {sortedOrders?.length !== 0 ? (
        ...
        <EmptyPage header={noOrdersHeader} description={noOrdersMessage}>
          <Button text={"В корзину"} onClick={() => navigation.push("/cart")} />
        </EmptyPage>
      )}
    </UserProfileWrapper>
  );
