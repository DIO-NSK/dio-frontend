...
import LegalConfirmationCodePopup from "@/components/organisms/popups/authorization/confirmation-code-popup/legal/LegalConfirmationCodePopup";
...
const RegisterForm = () => {
  ...
  const [captchaVisible, toggleCaptchaVisible, handleValidateForm, key, resetKey] = useSmartCaptcha<LegalEntityData>(
    schemaKeys,
    methods.trigger,
  ...
  return (
    <section className={"w-full -mt-7 sm:mt-0 sm:col-span-full flex flex-col gap-5 sm:gap-10"}>
      {confirmationPopupToggle.state && <LegalConfirmationCodePopup onClose={confirmationPopupToggle.toggleState} />}
        ...
        <Form className={"col-span-full flex flex-col gap-5"}>
          ...
          <BackgroundBlockWrapper header={"Дополнительная информация"}>
            {secondStepData.map((input, key) => (
              <ControlledTextInput classNames={{ wrapper: "col-span-1" }} theme={"filled"} {...input} key={key} />
            ))}
          </BackgroundBlockWrapper>
          <Button
