  ...
  <div className={cn("flex flex-row items-center gap-3", className)}>
    {product?.isNew ? (
      <span className={"px-3 py-2 rounded-lg bg-blue-500"}>
        <Text className={"uppercase text-xs sm:text-sm font-medium text-white"} text={"Новинка"} />
      </span>
    ) : null}
    ...
    {!product.inStock ? (
      <Chip className={"z-30 bg-gray-100"}>
        <Text className={"text-xs sm:text-sm uppercase text-text-gray"} text={"Нет в наличии"} />
      </Chip>
    ) : null}
  </div>
);
