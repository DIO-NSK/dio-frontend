  ...
  return (
    <div className={cn(wrapperCV)} onClick={handleSelectRequest}>
      ...
      <div className={"col-span-3 flex flex-col gap-1"}>
        {tableRow.item.message ? (
          <React.Fragment>
            <Text text={isExpanded ? tableRow.item.message : tableRow.item.message.slice(0, MAX_TEXT_LENGTH) + dots} />
            {tableRow.item.message.length > MAX_TEXT_LENGTH && (
              <TextButton onClick={handleSwitchExpanded} text={isExpanded ? "Скрыть" : "Развернуть"} />
            )}
          </React.Fragment>
        ) : (
          <Text className={"text-text-gray"} text={"—"} />
        )}
      </div>
    </div>
  );
  ...
  return (
    <TableWrapper {...props}>
      {props.tableContent.map((tableRow, rowKey) => {
        ...
        return <ServiceRow onSelect={props.onSelect} isSelected={isSelected} tableRow={tableRow} key={rowKey} />;
      })}
    </TableWrapper>
  );
};
