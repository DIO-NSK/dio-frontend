...
const OrderRowProductCard = ({ product, hasMore }: { product: ResponseCartItem; hasMore: boolean }) => {
  const wrapperStyles = ["w-full flex flex-col gap-2", { "pb-3 border-b-2 border-light-gray": hasMore }];
    ...
    <div className={cn(wrapperStyles)}>
      ...
      <section className={"w-full flex flex-row items-baseline gap-2"}>
        ...
        {product.discountPercent !== 0 && (
          <Text className={"text-text-gray text-sm line-through"} text={`${price.toFixed(2)} ₽`} />
        )}
      </section>
    </div>
  );
    ...
    <div className={cn(wrapperCV)} onClick={handleRowClick}>
      ...
      <div className={"col-span-1 flex flex-col gap-3"}>
        <Text
          ...
          className={"text-[14px] border-b-2 border-gray-100 pb-3"}
        />
        ...
        {products?.map(
          (product, key) =>
            (orderOpened.state || key == 0) && (
              <OrderRowProductCard product={product} hasMore={order.products?.length > 1} key={key} />
            ),
        )}
        {products?.length > 1 && (
          <TextButton text={orderOpened.state ? "Свернуть" : "Показать ещё"} onClick={orderOpened.toggleState} />
        )}
      </div>
    </div>
  );
  ...
  return (
    <TableWrapper classNames={{ header: "px-7" }} {...props}>
      {props.tableContent.map((tableRow, rowKey) => {
        const isSelected = props.selectedItems ? computeIsSelectedItem(props.selectedItems, tableRow) : undefined;
