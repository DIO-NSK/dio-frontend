  ...
  return (
    <div className={cn(wrapperCV)} onClick={handleSelectRequest}>
      ...
      <div className={"col-span-5 flex flex-col gap-1"}>
        <Text text={isExpanded ? tableRow.item.comment : tableRow.item.comment.slice(0, MAX_TEXT_LENGTH) + dots} />
        {tableRow.item.comment.length > MAX_TEXT_LENGTH && (
  ...
  return (
    <TableWrapper {...props}>
      {props.tableContent.map((tableRow, rowKey) => {
        ...
        return <CallRequestRow onSelect={props.onSelect} isSelected={isSelected} tableRow={tableRow} key={rowKey} />;
      })}
    </TableWrapper>
  );
};
