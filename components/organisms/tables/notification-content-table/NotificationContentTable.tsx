  ...
  return (
    <div className={cn(wrapperCV)} onClick={handleSelectItem}>
      <div className={"w-fit flex flex-row items-center gap-5"}>
        {props.tableRow.item.type !== "Проблема" && <Checkbox isSelected={props.isSelected} />}
        <div className={"flex flex-col gap-1"}>
          <Text text={props.tableRow.item.type} className={cn("text-lg", textCV)} />
          {props.tableRow.item.message && <Text text={props.tableRow.item.message} className={"text-text-gray"} />}
        </div>
      </div>
      <TextButton className={cn("hoverable pointer", buttonCV)} text={"Перейти"} onClick={handleButtonClick} />
    </div>
  );
};
