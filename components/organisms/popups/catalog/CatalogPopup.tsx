...
const SubcategoryItem = ({ onClose, text, urlMask }: { text: string; urlMask: string; onClose: () => void }) => {
  return (
    <Link href={`/catalog/${urlMask}`} onClick={onClose}>
      <Text className={"text-base text-text-gray pointer hoverable hover:text-link-blue"} text={text} />
    </Link>
  );
};
...
const CatalogPopup = () => {
  ...
  const tabs: TabBarItem[] = catalog.length
    ? catalog
        ...
        {activeSection && (
          <div className={"px-3 bg-white flex flex-row gap-8"}>
            ...
            <div className={"min-w-[300px] flex pt-3 flex-col gap-5"}>
              {activeTab && <Text className={"text-lg text-black font-medium"} text={activeTab.text} />}
