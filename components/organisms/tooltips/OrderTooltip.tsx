...
const OrderTooltip = ({ open, children }: TooltipProps<AdminOrder>) => {
  ...
  const rowCV: ClassValue[] = ["w-full flex flex-row items-center gap-3", "hoverable pointer hover:text-link-blue"];
      ...
      title={
        <div className={"p-4 flex flex-col gap-5"}>
          {tooltipBlocks.map((tooltipBlock, blockKey, array) => {
            ...
            return (
              <div key={blockKey} className={"w-full flex flex-col gap-5"}>
                ...
                <div className={cn("flex flex-col gap-5", blockCV)}>
                  {tooltipBlock.items.map((tooltipItem, itemKey) => (
                    <div className={cn(rowCV, tooltipItem.className)} onClick={tooltipItem.action} key={itemKey}>
                      {tooltipItem.icon}
