...
const AdminPanelSaleRuleBlock = () => {
  ...
  const { fields, append, remove, swap } = useFieldArray({ control, name: "ruleList" });
  ...
  return (
    <AdminPanelBlockWrapper className={"mx-0 px-7"}>
      <HeaderDescriptionButtonRow
        ...
        descr={
          "Введите каждое правило в отдельном поле для того, чтобы информация" +
      ...
      <section className={"w-full flex flex-col gap-5"}>
        <DndContext onDragEnd={handleChangeOrder} collisionDetection={closestCenter}>
      ...
      {errors?.ruleList?.root ? (
        <Text text={errors?.ruleList?.root?.message as string} className={"text-info-red text-[14px]"} />
      ) : null}
