...
const AdminPanelSearchbarBlock = (props: AdminPanelSearchbarBlockProps) => {
  ...
  const defaultProductIdRows =
    (props.defaultProducts?.map((product) => ({
      product: {
        ...product,
    ...
    <AdminPanelBlockWrapper className={"mx-0 px-7"}>
      ...
      <section className={"w-full flex flex-col gap-5"}>
        <DndContext onDragEnd={handleChangeOrder} collisionDetection={closestCenter}>
          ...
          >
            {productRows.map((productRow, index) => (
              <SortableItemWrapper sequenceNumber={productRow.product?.id as number} key={productRow.product?.id}>
                <DraggableRowWrapper
        ...
        {errors?.productIdList ? (
          <Text text={"Все поля не должны быть пустыми"} className={"text-info-red text-[14px]"} />
        ) : null}
      </section>
