  ...
  return (
    <section className={getClassName(isStatic)}>
      ...
      <div className="flex flex-row gap-5">
        <ControlledTextArea
          errors={
            (errorObject as any)?.keywords
              ? ({ message: "Количество ключевых слов не может быть больше 20." } as any)
              : null
          }
          hintText={{
            hintMessage:
              "Длина должна быть не больше 20 слов. Каждое ключевое слово должно разделяться от другого запятой. Любая последовательность символов до запятой воспринимается как одно ключевое слово.",
            type: "neutral",
          }}
