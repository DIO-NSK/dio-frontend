...
const itemCV: ClassValue[] = ["flex flex-row items-center gap-4 group", "rounded-xl pointer hoverable"];
  ...
  return (
    <div className={"w-full px-10 flex flex-col gap-5 pt-5 border-t-2 border-light-gray"}>
      {footerData.map((item, key) => {
        ...
        return (
          <div key={key} className={cn(itemCV)} onClick={item.action}>
...
const AdminPanelSidebar = () => {
  ...
  const adminTabBarData: TabBarItem[] = [
    ...
    { text: "SEO", icon: <FiLink size={"18px"} />, path: "/admin/seo" },
  ];
  ...
  return (
    <div className={cn(wrapperCV)}>
      <div className={"w-full flex flex-col gap-5"}>
        {isFolded ? <DIOLogoSmall size={47} className={"mx-9"} /> : <DIOLogoBig className={"mx-9"} />}
        <AdminTabBar tabs={adminTabBarData} activeTab={activeTab} setActive={handleTabClick} />
      </div>
