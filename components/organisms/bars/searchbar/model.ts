...
const searchCatalog = async (searchName: string) => {
  return unauthorizedApi
    .get("/catalogue/search", { params: { name: searchName } })
...
sample({
  ...
  source: $catalog,
  fn: (catalog: CatalogItem[], tabItem: TabBarItem) => catalog.find((section) => section.name === tabItem.text)!!,
  target: $activeSection,
});
...
const searchCatalogByName = async (name: string): Promise<ResponseSearchCatalog> => {
  return unauthorizedApi
    .get("/catalogue/search", { params: { name: name } })
    .then((response) => response.data)
    .catch((error) => {
      throw Error(error.response.data.message);
    });
