...
const ActionIcon = (props: SnackbarProps) =>
  props.success ? (
    <FiX
      ...
      onClick={() => (props?.action ? props.action() : props.onClose())}
      size={"24px"}
    />
  ) : (
    <FiX className={"text-info-red hoverable hover:text-red-700 pointer"} onClick={props.onClose} size={"24px"} />
  );
  ...
  return (
    ...
    >
      <section className={"w-full flex flex-col gap-2"}>
        ...
        {props.message && <Text className={cn("text-base max-w-[400px]", textClassName)} text={props.message} />}
      </section>
    </StyledSnackbar>
  );
};
