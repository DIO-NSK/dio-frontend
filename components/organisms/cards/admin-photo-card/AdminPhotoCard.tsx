...
const PhotoCard = (
  ...
  return (
    <div
      className={cn(
        "col-span-1 relative h-[150px] rounded-xl border-2 border-light-gray overflow-clip",
        props.className,
      )}
    >
      <img src={resolvedImage} alt={"Фотография продукта"} className={"w-full h-full object-scale-down"} />
      <div className={"absolute flex flex-row items-center gap-3 z-10 top-5 right-5"}>
        {(props.editable || props.draggable) && (
          <React.Fragment>
            {props.editable && <SquareIcon icon={<FiEdit size={"18px"} />} onClick={props.onEdit} />}
            <div
        ...
        {props.canDelete && <ClosePopupButton className={cn(closeButtonCV)} onClose={props.onDelete} />}
      </div>
    </div>
  );
};
