...
import { ShoppingCartProductCardProps } from "@/components/organisms/cards/shopping-cart-product-card/ShoppingCartProductCard.types";
  ...
  return (
    <section className={"md:hidden flex flex-col gap-2"}>
      ...
      <div className={"flex flex-row items-baseline gap-2"}>
        ...
        />
        {props.card.discountPercent !== 0 && (
          <Text text={`${(price * props.card.quantity).toFixed(2)} ₽`} className={"text-text-gray line-through"} />
        )}
      </div>
      {canInteract ? (
        <div className={"flex flex-row items-center gap-7"}>
        ...
        <Text className={"text-base sm:text-lg text-text-gray"} text={`${props.card.quantity} шт.`} />
      )}
    </section>
  );
};
