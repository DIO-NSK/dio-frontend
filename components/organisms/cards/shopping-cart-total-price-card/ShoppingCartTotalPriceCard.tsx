...
const ShoppingCartTotalPriceCard = ({
  ...
  return (
    <StickyCardWrapper
      startCol={"md:col-start-9 md:col-span-4 md:p-5 md:gap-4 xl:gap-5 xl:p-7 xl:col-start-10 xl:col-span-3"}
    >
      ...
      <div className={cn(rowCV)}>
        ...
        <Text text={`${totalActualPrice.toFixed(2)} ₽`} className={"text-[24px] font-medium text-link-blue"} />
      </div>
      {!isValidCart ? (
        <Text
      ...
      <Button hasSpinner={false} disabled={!(hasItemInStock || isValidCart)} text={buttonText} onClick={onClick} />
    </StickyCardWrapper>
  );
};
