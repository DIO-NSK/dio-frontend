...
const CheckoutCardItem = ({
  ...
  productItem: ResponseCartItem | ProfileOrderItem | ResponseCartSaleItem;
}) => {
    ...
    <div className={"pt-5 w-full flex flex-col gap-1"}>
      <div className={"w-full flex flex-row items-baseline justify-between"}>
        <div className={"flex flex-row items-baseline gap-2"}>
          <Text text={`${actualPrice.toFixed(2)} ₽`} className={"text-link-blue font-medium"} />
          {totalOldPrice !== 0 && (
            <Text text={`${totalOldPrice.toFixed(2)} ₽`} className={"text-text-gray line-through text-sm"} />
          )}
        </div>
        <Text text={`${productItem.quantity} шт.`} className={"text-text-gray text-sm"} />
      </div>
      <Text text={productItem.name} className={"font-medium"} />
    </div>
  );
};
...
const CheckoutCard = ({ cart }: { cart: ResponseUserCart }) => {
  ...
  return (
    <StickyCardWrapper startCol={"lg:col-start-9 lg:col-span-4 xl:col-start-10 xl:col-span-3 lg:mt-[40px] xl:mt-0"}>
      ...
      {expanded.state && (
        <section className={"flex -mt-5 flex-col gap-4 divide-y divide-light-gray"}>
