    ...
    <section className={cn("w-full flex flex-col gap-5", className)}>
      <section className={"w-full flex flex-row gap-5"}>
        <img
          src={promo.mainImage}
          className={
            "w-[76px] h-[60px] md:h-[90px] md:w-[120px] xl:h-[90px] xl:w-[180px] shrink-0 rounded-lg object-scale-down"
          }
        ...
        <div className={"w-full flex flex-col gap-5 sm:gap-2"}>
          ...
          <MoreButton setExpanded={isExpanded.toggleState} isExpanded={isExpanded.state} text={"Подробнее"} />
        </div>
      </section>
      {isExpanded.state && (
        <section className={"w-full flex flex-col gap-5 pt-5 border-t-2 border-light-gray"}>
          {isExpanded.state &&
