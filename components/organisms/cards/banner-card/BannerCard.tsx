...
const BannerCard = () => {
  ...
  return (
    <div className={cn(wrapperCV)} onClick={() => console.log("Все товары")}>
      <img className={"mt-1 sm:m-0 w-10 sm:w-[50px] h-10 sm:h-[50px] object-cover"} src={DIOLogo.src} alt={"/"} />
      <div className={"flex flex-col gap-1 sm:gap-[10px]"}>
        ...
        />
        <Text className={"text-[15px] sm:text-[20px] text-text-gray"} text={"Посмотреть все товары"} />
      </div>
    </div>
  );
};
