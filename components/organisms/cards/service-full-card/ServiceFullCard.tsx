...
const PriceRow = ({ price, isPerDay }: { price?: number; isPerDay?: boolean }) => {
  if (price !== undefined && price !== 0) {
    ...
    return (
      <Text
        text={"Бесплатно"}
        className={"w-full text-[20px] md:text-[22px] xl:text-[24px] font-semibold text-link-blue"}
      />
    );
  }
};
...
const RentTimeBlock = ({ rentTime }: { rentTime: { name: string; value: string }[] }) => {
  return (
    <ServiceBlockWrapper header={"Срок аренды"}>
      {rentTime.map((item, key) => {
        return (
          <section
            className={"col-span-3 flex flex-row md:flex-col md:gap-2 lg:flex-row items-baseline justify-between"}
            key={key}
          >
...
const ServiceFullCard = ({ card }: { card: ServiceCardDTO }) => {
