  ...
  return (
    <div className={"hidden w-full sm:flex flex-row items-center justify-between"}>
      <div className={"flex flex-row items-baseline gap-2 sm:gap-4"}>
        ...
        {card.deadline && <Text text={`Акция до ${card.deadline}`} className={"text-base text-text-gray"} />}
      </div>

      <div className={"flex flex-row items-center gap-5"}>
        <Text text={`${card.amount} шт.`} className={"text-text-gray text-lg"} />
        <div className={"flex flex-row items-center gap-[15px]"}>
          <Tooltip title={"Данная услуга идёт в подарок в рамках акции"} variant={"outlined"} color={"primary"} arrow>
            <button>
              <FiInfo size={"20px"} className={"text-text-gray hoverable pointer hover:text-link-blue"} />
            </button>
          </Tooltip>
  ...
  return (
    <section className={"sm:hidden w-full flex flex-col gap-1"}>
      <div className={"flex flex-row items-center gap-2"}>
        ...
        <Tooltip title={"Данная услуга идёт в подарок в рамках акции"} variant={"outlined"} color={"primary"} arrow>
          <button>
            <FiInfo size={"18px"} className={"text-text-gray hoverable pointer hover:text-link-blue"} />
          </button>
        </Tooltip>
      </div>
      <Text text={card.header} />
      {card.deadline && <Text text={`Акция до ${card.deadline}`} className={"text-[14px] text-text-gray"} />}
    </section>
  );
};
