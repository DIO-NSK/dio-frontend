  ...
  return (
    <section className={"flex flex-col gap-7"}>
      <HeaderRow
        ...
        theme={"bordered"}
        header={"Товары"}
        rightContent={<ChevronButton isExpanded={productsToggle.state} setExpanded={productsToggle.toggleState} />}
      />
      {productsToggle.state && (
        <div className={"w-full flex flex-col -mt-7"}>
          {catalog?.productList.map((product, index) => {
            ...
            return (
              <Link className={cn(productRowCV)} href={`/product/${(product as any).urlMask}`} key={index}>
      ...
      <HeaderRow
        ...
        theme={"bordered"}
        header={"Категории"}
        rightContent={<ChevronButton isExpanded={categoriesToggle.state} setExpanded={categoriesToggle.toggleState} />}
      />
      {categoriesToggle.state && (
        <section className={"w-full flex flex-col -mt-7"}>
          {catalog?.categoryList.map((category, index) => (
            <div className={"w-full flex flex-col gap-2 py-7 px-5 border-b-2 border-light-gray"} key={index}>
