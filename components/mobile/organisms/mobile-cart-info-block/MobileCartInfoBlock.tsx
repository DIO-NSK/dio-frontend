  ...
  return (
    <section className={"md:hidden w-full flex flex-col gap-5"}>
      {props.infoBlockData.map((element, key) => (
        <div
          key={key}
          className={"w-full flex flex-row items-baseline justify-between pb-5 border-b-2 border-light-gray"}
        >
      ...
      <Button classNames={{ button: props.className }} onClick={props.onSubmit} text={props.buttonText} />
    </section>
  );
};
