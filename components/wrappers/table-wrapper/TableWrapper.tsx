...
const Header = (props: TableWrapperProps) => {
  const wrapperCV: ClassValue[] = ["w-full px-7 grid grid-cols-8", "gap-x-7 pb-7 border-b-2 border-light-gray"];

  return (
    <div className={cn(wrapperCV, props.classNames?.header)}>
      {props.tableHeader?.map((item, key) => (
        <Text key={key} text={item.text} className={cn("text-text-gray", item.width)} />
      ))}
    </div>
  );
};
...
const Content = (props: TableWrapperProps) => {
  return <div className={cn("w-full flex flex-col", props.classNames?.content)}>{props.children}</div>;
};
