...
const SearchbarIconButtonList = () => {
  ...
  const [cart, getCart, userCredentials, getUserCredentials] = useUnit([
    $cart,
    getCartEvent,
    $userCredentials,
    getUserCredentialsEvent,
  ]);
  ...
  const buttonListData = [
    {
      ...
      onClick: () => {
        userCredentials ? router.push("/profile") : switchPopupState("login");
      },
    },
    {
      name: "Избранное",
    ...
    },
    {
      ...
      icon: (
        <div className={"relative"}>
          ...
          {cart && itemsInCart !== 0 ? (
            <Badge
              className={"absolute px-[5px] items-center justify-center bottom-2 left-4 z-10"}
              number={itemsInCart}
            />
          ) : null}
        </div>
  ...
  return (
    <div className={"flex flex-row items-center gap-5 xl:gap-[30px]"}>
      {buttonListData.map((item, key) => {
        ...
        return (
          <IconTextButton className={cn(itemCV)} icon={item.icon} text={item.name} onClick={item.onClick} key={key} />
        );
      })}
    </div>
  );
