  ...
  return (
    <div className={"flex flex-row gap-2"}>
      {variants?.map((variant, key) => {
        ...
        return (
          <div className={cn(variantCV)} onClick={() => onSelectVariant?.(variant)} key={key}>
  ...
  return (
    <div className={cn("w-full relative group", props.classNames?.wrapper)}>
      <div className={cn(absoluteWrapperCV)}>
        ...
        {props.value ? <FiX className={cn(iconCV)} onClick={handleClear} /> : <FiSearch className={cn(iconCV)} />}
      </div>
      <TextInput
        ...
        {products?.map((product, index, array) => {
          ...
          return (
            <LinkWrapper
              ...
              key={index}
            >
              <section className={"flex flex-row items-center gap-5"} onClick={() => handleSelectProduct(product)}>
                <img
                ...
                <div className={"w-full flex flex-col gap-2"}>
                  <div className={"flex flex-row items-baseline gap-2"}>
                    <Text className={"text-link-blue font-medium"} text={`${newPrice.toFixed(2)} ₽`} />
                    {product.discountPercent !== 0 && (
                      <Text className={"text-sm text-text-gray line-through"} text={`${price.toFixed(2)} ₽`} />
                    )}
                  </div>
                  <Text text={product.name} />
                </div>
              </section>
              {props.selectedElement?.id === product.id && <FiCheck size={"20px"} className={"text-link-blue"} />}
            </LinkWrapper>
          );
        })}
      </section>
    ...
    <section className={"w-full flex flex-col gap-5"}>
      ...
      <section className={"w-full flex flex-col"}>
        {categories.map((category, index) => (
          <div className={"w-full flex flex-col gap-2 p-7 pt-0 border-b-2 border-light-gray"} key={index}>
...
const NotFoundMessage = ({ catalog }: { catalog: ResponseSearchCatalog }) => {
  if (catalog.categoryList.length === 0 && catalog.productList.length === 0) {
    return <Text text={"Мы не смогли найти товар или категорию по вашему запросу"} className={"p-7 text-text-gray"} />;
  }
};
...
const PopoverList = <T,>(props: PopoverProps<T>) => {
  ...
  if (catalog)
    return (
      <section
        className={
          "absolute overflow-y-auto customScrollbar max-h-[400px] z-20 top-[80px] w-full flex flex-col gap-5 rounded-xl bg-white shadow-2xl"
        }
      >
...
const SearchInput = <T,>({ hasPopover = false, ...props }: SearchbarProps<T>) => {
  ...
  return (
    <section ref={ref} className={cn("w-full relative flex flex-col gap-5", props.classNames?.mainWrapper)}>
