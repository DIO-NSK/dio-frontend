...
const InnerInput = <T extends FieldValues>({ theme = "outlined", ...props }: ControlledTextInputProps<T>) => {
  const [isPasswordState, setIsPasswordState] = useState<boolean>(props.isPassword ?? false);
      ...
      {(methods: UseFormReturn<T, any, T>) => (
        <div className={"w-full relative"}>
          ...
          <div className={"absolute z-10 top-1/3 right-7"}>
            ...
            {props.isPassword && <EyeButton isOpen={isPasswordState} setOpen={setIsPasswordState} />}
          </div>
        </div>
      )}
    </ConnectForm>
...
const ControlledTextInput = <T extends FieldValues>({ hasInnerError, ...props }: ControlledTextInputProps<T>) => {
  return (
    <ControlledInputWrapper hasInnerError={hasInnerError} props={props}>
      <InnerInput {...props} />
      <div className={"absolute right-5 top-[52px]"}>{props?.button}</div>
