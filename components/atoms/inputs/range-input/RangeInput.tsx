...
const RangeInput = (props: RangeInputProps) => {
  ...
  const inputList = [
    ...
    },
    {
      ...
      className:
        "w-full p-[20px] text-right rounded-r-xl border-2 border-l-0 border-light-gray bg-bg-light-blue focus:outline-0",
      placeholder: "до",
      value: props.toValue,
    },
  ];
    ...
    <div className={"w-full flex flex-col"}>
      <div className={"w-full flex flex-col gap-2"}>
        ...
        <div className={"w-full flex flex-row items-center gap-[0px]"}>
          {inputList.map((input) => (
            <input {...props} {...input} onKeyDown={handleKeyPress} pattern={"[0-9]*"} type={"text"} />
          ))}
        </div>
      </div>
      <Slider
