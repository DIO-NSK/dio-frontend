...
const buttonStyles = {
  button: cn(["fixed w-[calc(100%-40px)] bottom-5 left-5 z-30 justify-between", "px-5 py-5 sm:hidden flex"]),
};
    ...
    <Button
      ...
      rightContent={
        <div className={"flex flex-row items-baseline gap-3"}>
          <Text className={"text-white"} text={`${newPrice.toFixed(2)} ₽`} />
          {itemDiscountPercent !== 0 ? (
            <Text className={"text-white text-opacity-50 line-through"} text={`${price.toFixed(2)} ₽`} />
          ) : null}
        </div>
      }
